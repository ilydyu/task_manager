...
func (s *Suite) TestGetTaskHistory() {
	...
	history, err := s.client.GetTaskHistory(ctx, task.ID, user.Token)

	s.NoError(err)

	s.Equal(task.ID, history.History[0].TaskID)
	s.Equal("create", history.History[0].Action)
}
