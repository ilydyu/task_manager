...
func (s *Suite) TestUpdateTask() {
	...
	s.T().Run("success update task", func(t *testing.T) {
		deadline := time.Now().Add(24 * time.Hour).Truncate(time.Second)
		task, err := s.client.UpdateTask(ctx, task.ID, user.ID, "New Task", "New desc", "done", "high", user.Token, deadline)

		s.NoError(err)
		s.Equal("New Task", task.Title)
		s.Equal("New desc", *task.Description)
		s.Equal(domain.TaskStatusDone, task.Status)
		s.Equal(domain.TaskPriorityHigh, task.Priority)
		s.Equal(team.ID, task.TeamID)
		s.Equal(user.ID, *task.AssigneeID)
		s.Equal(user.ID, task.CreatedBy)
		s.Equal(deadline, *task.Deadline)
	})

	s.T().Run("foreign user trying update task", func(t *testing.T) {
		deadline := time.Now().Add(24 * time.Hour).Truncate(time.Second)
		_, err := s.client.UpdateTask(ctx, task.ID, thirdUser.ID, "task", "desc", "backlog", "low", thirdUser.Token, deadline)
